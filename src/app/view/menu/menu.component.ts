...
export class MenuComponent implements OnInit {
  ...
  laodCoffes(): void {
    const storedCoffees = sessionStorage.getItem("coffes");
    
    if (storedCoffees) {
      // Si hay cafés en sessionStorage, los usamos
      this.coffes$ = new Observable(observer => {
        observer.next(JSON.parse(storedCoffees));
        observer.complete();
      });
    } else {
      // Si no hay cafés en sessionStorage, los cargamos del backend
      this.getCoffes();
    }
  }
